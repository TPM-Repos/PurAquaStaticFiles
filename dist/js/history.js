 ...
 */
async function getSpecificationsWithQuery(
	oDataQueryString = "",
	clearList = false,
) {
...
async function getSpecificationProperties(specificationId) {
	...
	const output = document.querySelector(
		`.history-item[data-id='${specificationId}'] .item-properties`,
	)
...
function setExpandToggleState() {
	...
	if (expandedView) {
		icon.innerHTML =
			'<svg class="icon"><use xlink:href="dist/icons.svg#collapse" /></svg>'
	...
	icon.innerHTML =
		'<svg class="icon"><use xlink:href="dist/icons.svg#expand" /></svg>'

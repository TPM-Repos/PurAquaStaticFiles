...
async function renderDetails(specification) {
	...
	if (
		!storedSpecification ||
		!objectsEqual(specification, storedSpecification)
	) {
		...
		const markup = `
            ${
				status &&
				`<div class="detail-item detail-status"><div>Status</div><div class="status-tag status-${normalizeString(
					status,
				)}" title="${splitOnUpperCase(status)}">${splitOnUpperCase(
					status,
				)}</div></div>`
			}
            ${
				dateCreated &&
				`<div class="detail-item detail-created"><div>Created</div><div>${dateCreated}</div></div>`
			}
            ${
				dateEdited &&
				`<div class="detail-item detail-edited"><div>Modified</div><div>${dateEdited}</div></div>`
			}
        `
		...
		if (messages.length > 0) {
			...
			setTimeout(function () {
				const confirmation = window.confirm(
					`If you continue, the following actions will take place:\n${messages.join(
						"\n",
					)}`,
				)
...
async function getProperties() {
	try {
		const properties = await client.getSpecificationProperties(
			GROUP_ALIAS,
			QUERY_SPECIFICATION_ID,
		)
		...
		if (properties && !isEmpty(properties)) {
			...
			if (
				!storedProperties ||
				!objectsEqual(properties, storedProperties)
			) {
...
async function getDocuments() {
	try {
		// Get Documents
		const documents = await client.getSpecificationDocuments(
			GROUP_ALIAS,
			QUERY_SPECIFICATION_ID,
		)
...
function renderDocuments(documents) {
	...
	for (item of documents) {
		...
		if (
			imageFormats.indexOf(item.extension.toLowerCase()) >= 0 &&
			item.fileExists
		) {
...
function updateGeneratingMessage() {
	if (generatingCount > 0) {
		generatingText.innerHTML = `${
			documentCount - generatingCount + 1
		}/${documentCount}`
...
function renderEmptyDocuments() {
	documentsList.innerHTML =
		'<div class="empty-documents">No documents available at this time.</div>'
	detailDocuments.style.opacity = ""
}
...
async function renderImage(image) {
	...
	const imgUrl = await client.getSpecificationDocumentUrl(
		GROUP_ALIAS,
		QUERY_SPECIFICATION_ID,
		id,
		resourceName,
...
function renderDocument(file) {
	...
	const documentUrl = client.getSpecificationDocumentUrl(
		GROUP_ALIAS,
		QUERY_SPECIFICATION_ID,
		id,
		resourceName,

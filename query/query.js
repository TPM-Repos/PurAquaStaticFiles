...
class SessionManager {
	...
	async handleAutoLogin() {
		...
		const restartSession =
			REQUIRE_NEW_SESSION || (REQUIRE_EXACT_ALIAS && aliasIsDifferent)
...
class QueryManager {
	...
	async retrieveQueryData() {
		const queryProjectName =
			urlQuery.get("run") ||
			urlQuery.get("Run") ||
			urlQuery.get("project") ||
			urlQuery.get("Project")
		this.projectName = queryProjectName
			? queryProjectName
			: config.query.defaultProjectName
		this.driveAppAlias =
			urlQuery.get("driveapp") ||
			urlQuery.get("driveApp") ||
			urlQuery.get("DriveApp")
		this.specificationId =
			urlQuery.get("specification") || urlQuery.get("Specification")
		this.operationName =
			urlQuery.get("operation") || urlQuery.get("Operation")
		this.transitionName =
			urlQuery.get("transition") || urlQuery.get("Transition")
		...
		for (const [key, value] of urlQuery) {
			if (
				!key.startsWith(QUERY_PREFIX_CONSTANTS) &&
				!key.startsWith(QUERY_PREFIX_MACROS)
			) {
				continue
			}
	...
	async validateQueryData() {
		...
		if (!this.projectName && !this.driveAppAlias && !this.specificationId) {
			displayErrorMessage(
				"No Project/DriveApp name or Specification Id provided.",
			)
			return false
		}
		...
		if (!this.specificationId || this.specificationId == "") {
			if (this.operationName) {
				displayErrorMessage(
					"Operation provided without Specification Id.",
				)
				return false
			}
			...
			if (this.transitionName) {
				displayErrorMessage(
					"Transition provided without Specification Id.",
				)
				return false
			}
		}
	...
	async performOperation() {
		...
		try {
			...
			debug(error, true)
			displayErrorMessage(
				`Could not perform Operation "${this.operationName}".`,
			)
		}
	}
	...
	async performTransition() {
		...
		try {
			...
			if (
				specification.stateType == "Running" ||
				specification.stateType === RUNNING_STATE_ID
			) {
			...
			debug(error, true)
			displayErrorMessage(
				`Could not perform Transition "${this.transitionName}".`,
			)
		}
	}
...
function redirectToLogin(notice, state = "error", addReturnUrl = false) {
	...
	if (addReturnUrl && config.loginReturnUrls) {
		if (config.folder && config.folder !== "") {
			logoutQuery = `returnUrl=${
				config.folder
			}/query/${encodeURIComponent(window.location.search)}`
		} else {
			logoutQuery = `returnUrl=query/${encodeURIComponent(
